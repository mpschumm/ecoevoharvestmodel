...
plot(0:(length(ratios)-1), ratios, xlab="length (cm)", ylab="Ratio of energy to be allocated to reversible:irr.", type="l", ylim=c(lambda_min, lambda_max))
abline(h=1.13)
...
curve(exp(-70*x), from =0, to=1.5, xlab="(E/S), or Lambda_actual", ylab="Additional natural mortality", main="Body condition-related mortality with body condition (E/S)")
```

(Eq. 13) Selectivity = $\large \frac{1}{1+e^{-q_f*(length(cm)-mincatchsize)}}$

```{r}
curve(1/(1+exp(-0.1*(x - 50))), from =0, to=150, xlab="Length in cm.", ylab="proportion maximum fishing mortality", main="Selectivity of harvest")
```
...
```{r, echo=F}
total_r_nh <- vector(mode="numeric")
g_1_r_nh <- vector(mode="numeric")
g_2_r_nh <- vector(mode="numeric")
selection_r_nh <- vector(mode="numeric")

for (rrun_counter in 1:10) {

# Need to take this loop out, integrate into rest of code
random_resources <- vector(mode="numeric", length=runtime)
for (r in 1:runtime) {
  random_resources[r] <- max(rnorm(1,1,sd=0.8),0)
}
...
total_r_nh <- c(total_r_nh, approach[seq(round(add_genotypes/timescale),length(approach), by=4)])
# Plot population dynamics
...
g_1_r_nh <- c(g_1_r_nh, approach[seq(round(add_genotypes/timescale),length(approach), by=4)])
cols <- c(3,4,5,6)
for (j in 2:genotypes) {
  approach <- vector(mode="numeric", length=runtime)
  for (i in 1:runtime) {
  ...
  g_2_r_nh <- c(g_2_r_nh, approach[seq(round(add_genotypes/timescale),length(approach), by=4)])
}
legend("topright",legend=c("all", "large maturer","small maturer"), lty=1:3)

# Get mean parent genotypes
...
# selection_r_nh <- c(selection_r_nh, (mean_offspring_genotypes-mean_parent_genotypes))

selection_r_nh <- c(selection_r_nh, (mean_offspring_genotypes))
...
```{r, echo=F}
total_r_h <- vector(mode="numeric")
g_1_r_h <- vector(mode="numeric")
g_2_r_h <- vector(mode="numeric")
selection_r_h <- vector(mode="numeric")
...
total_r_h <- c(total_r_h, approach[seq(round(add_genotypes/timescale),length(approach), by=4)])
# Plot population dynamics
...
lines(approach[seq(round(add_genotypes/timescale),length(approach), by=4)], lty=2, pch=19, type="l")
g_1_r_h <- c(g_1_r_h, approach[seq(round(add_genotypes/timescale),length(approach), by=4)])
cols <- c(3,4,5,6)
for (j in 2:genotypes) {
  approach <- vector(mode="numeric", length=runtime)
  for (i in 1:runtime) {
  ...
  lines(approach[seq(round(add_genotypes/timescale),length(approach), by=4)], lty=cols[j-1], pch=19)
  g_2_r_h <- c(g_2_r_h, approach[seq(round(add_genotypes/timescale),length(approach), by=4)])
}
legend("topright",legend=c("all", "large maturer","small maturer"), lty=1:3)
...
selection_r_h <- c(selection_r_h, (mean_offspring_genotypes-mean_parent_genotypes))
...
```{r}
# i_0 must be a year where there was recruitment
i_0 = 443
repro_vec = vector(length=longevity, mode="numeric")
survivor_vec = vector(length=longevity, mode="numeric")
length_vec_1 = vector(length=100, mode="numeric")
cond_vec_1 = vector(length=100, mode="numeric")
w = genotype1w
r = genotype1r
age_dist <- age_dist_1
...
for (i in 2:longevity) {
  ...
  survivor_vec[i] <- age_dist[i,i_0+i]/(survivor_vec[1])
   length_vec_1[i] <- age_dist[i+(2*longevity),i_0+i]
   cond_vec_1[i] <- 100*(((age_dist[i+(longevity),i_0+i]))+structural_mass(age_dist[i+(2*longevity),i_0+i]))/((100*age_dist[i+(2*longevity),i_0+i])^3)
}
...
length_vec = vector(length=100, mode="numeric")
cond_vec = vector(length=100, mode="numeric")
w = genotype2w
r = genotype2r
age_dist <- age_dist_2
survivor_vec[1]<-age_dist[1,i_0+1]
repro_vec[1] <- 0
for (i in 2:longevity) {
  # Should really be relative recruitment
  repro_vec[i] <- max(0, (w*age_dist[i+(longevity),i_0+i] - (r_0*structural_mass(age_dist[i+(2*longevity),i_0+i]))^(r_1)))
 survivor_vec[i] <- age_dist[i,i_0+i]/(survivor_vec[1])
 length_vec[i] <- age_dist[i+(2*longevity),i_0+i]
 cond <- 100*(((age_dist[age-1+(longevity),t-1]))+structural_mass(age_dist[age-1+(2*longevity),t-1]))/((100*age_dist[age-1+(2*longevity),t-1])^3)
 cond_vec[i] <- 100*(((age_dist[i+(longevity),i_0+i]))+structural_mass(age_dist[i+(2*longevity),i_0+i]))/((100*age_dist[i+(2*longevity),i_0+i])^3)
}
...
plot((length_vec[2:length(survivor_vec)])-(length_vec_1[2:length(survivor_vec_1)]), ylab="Length of genotype 2 - genotype 1", main="Length advantage/disadvantage of\ngenotype 2 over 1", xlab="Age")
...
^ We dealt with that, but should we look into mortalities from condition as well, where do they come from? Also, mortalities in "growth_curves.R"?
 
There needs to be a penalty for maturing at a smaller size and the penalty has to be that the fish never reaches longest length, for energy acquisition and predator avoidance... but is not happening here. Am I bringing in too much energy? 

it's right after reproducing, so it makes sense
well...

# THE ORDER OF MORTALITY (NOT FOLLOWING REPRODUCTION) IS AN ISSUE
# MORTALITY SHOULD COME BEFORE BOTH?

 # Why is survivorship increased with genotype/not changing with resource changes?
