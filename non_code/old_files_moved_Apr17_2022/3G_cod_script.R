...
recruitment <- function(age, ages, t) {
  ...
  #   assign(paste("age_dist_",g,sep=""),age_dist, envir = .GlobalEnv) 
  # }
  p<-(N0*relative_recruitment[1]+(1/2)*N0*relative_recruitment[2])/N0
  q <- 1-p
  alleles<-c(rep(1,round(p*N0*2)),rep(2,round(q*N0*2)))
  zygotes<-split(sample(alleles),rep(1:(length(alleles)/2),each=2))
  age_dist_1 = get(paste("age_dist_1"))
  age_dist_2 = get(paste("age_dist_2"))
  age_dist_3 = get(paste("age_dist_3"))
  age_dist_2[1,t]<-length(Filter(function(x) sum(unlist(x))==3, zygotes))
  age_dist_1[1,t]<-length(Filter(function(x) sum(unlist(x))==2, zygotes))
  age_dist_3[1,t]<-N0-age_dist_1[1,t]-age_dist_2[1,t]
  assign((paste("age_dist_1")), age_dist_1)
  assign((paste("age_dist_2")), age_dist_2)
  assign((paste("age_dist_3")), age_dist_3)
}
...
##### SCENARIO 2 (HARVEST - DETERMINISTIC)
...
legend("topright",legend=c("all", "large maturer","medium maturer", "small maturer"), lty=1:4, col=c("black","blue","red", "orange"))
