...
# NOTE! some of the references in comments to "resources" may may actually pertain to temperature because they are unchanged from the version of this code which does NOT use thermal variation, as this code file uses the same code to vary temperature stochastically rather than directly varying food resources.
# Setting up the crosses between different levels of the harvest and temperature variability variables
rand_resource_vec <- rep((sqrt(log(seq(0,2, length=12)^2 + 1))), each=4900,times=1) # Values input to seq() are in units of coefficient of variation of lognormal distribution of temperatures - they are back-converted here to SD of a Normal
fishing_vec <- rep(seq(0,1.8,by=1.8/13),each=350, times=12)
...
model_iteration <<- function(results_counter) {
  ...
  mapply(function(y) try(mat[1,y] <<- data_list[[y]]), y=c(1:6,12:13))
  ...
  # Unlike the versions of these code files which simulated variability in food resources, this code is designed to write output files to a folder as they are generated, rather than generating one large R object with all of the data to be analyzed by the "extracting_output.R" file. 
  # To run these files on a different machine/system/configuration, directories names' below must be changed.
  # Code for processing the .csv files generated by these is in the "plotting_code_thermal_analyses.R" file in the "figure_data+code" folder.
...
model_run <-function(list, x) {
  ...
  mapply ( function(growing) {
    ...
    random_adjustment <- ((random_adjustment - 1)/1000)+1
    ...
    parameters <- c(resource_r = resource_r, resource_K = resource_K, K_half=K_half, nu=nu)
