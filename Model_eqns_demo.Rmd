...
Equation for interconverting between the structural mass of an age cohort's individuals at the previous timestep, and those individuals' body length at the previous timestep, where S is structural mass in grams:
...
plot(0:(length(ratios)-1), ratios, xlab="length (cm)", ylab="lambda_l", type="l")
abline(h=1.3)
```

(Eq. 3) $\large I(S) = I_{0}S^{I_{1}}$, where I is energy intake, and $I_{0}$ and $I_{1}$ are constants.

The cost of maintenance, where E is reversible mass in grams:

(Eq. 4) $\large C_{m}(S,E) = c_{S}S + c_{E}E$

The net intake of energy, after maintenance has occurred:

(Eq. 5) $\large I_{net} = I(S) - C_{m}(S,E)$  
...
$\large E_t = E_{t-1} + I_{net}\times{e_E}$
where $e_E$ and $e_S$ are efficiencies for conversions between energy intake and reversible and structural mass respectively.

&nbsp;

(Eq. 7) **if $I_{net} > 0$ & $\lambda_{l} > \frac{E_{t-1}}{S_{t-1}}$:**

Calculate the amount of energy needed to bring Rev/Struct, or actual $\lambda$, up to $\lambda_{l}$.

$\large I_E = (\lambda_{l}S_{t-1}-E_{t-1})/e_{E}$

Give that amount (or as much of it as is possible) to reversible mass.

if $\large (I_E - I_{net} \geq 0)$:

$\>$$\>$$\>$ $\large E_{t-1} = I_{net} \times e_{E} + E_{t-1}$

$\>$$\>$$\>$ $\large I_{net} = 0$

if $\large (I_E - I_{net} < 0)$:

$\>$$\>$$\>$ $\large E_{t-1} = I_{E} \times e_{E} + E_{t-1}$

$\>$$\>$$\>$ $\large I_{net} = I_{net} - I_{E}$
...
(Eq. 8a) $\large E_{t} = E_{t-1} + \frac{\lambda_{l} \times e_S}{\lambda_{l} \times e_S + e_E}\times I_{net}\times e_E$

(Eq. 8b) $\large S_{t} = S_{t-1} + (1 - \frac{\lambda_{l} \times e_S}{\lambda_{l} \times e_S + e_E})\times I_{net}\times e_S$
...
Fraction of $I_{net}$ devoted to S if $l < l_{mat}$: $\frac{E}{(1+c_{j})*c_{j}*S}$

Fraction of $I_{net}$ devoted to S if $l > l_{mat}$: $\frac{E}{(1+c_{a})*c_{a}*S}$
...
The NON-SPAWNING cost of reproduction, in grams.
...
```{r}
curve(exp(-8*x), from =0, to=1, xlab="Length in m.", ylab="Additional natural mortality")
curve(exp(-7*x), from =0, to=1.5, xlab="Lambda_actual", ylab="Additional natural mortality")
```

(Eq. 13) Selectivity = $\large \frac{1}{1+e^{-q_f*(length(cm)-mincatchsize)}}$

```{r}
curve(1/(1+exp(-0.2*(x - 50))), from =0, to=150, xlab="Length in cm.", ylab="proportion maximum fishing mortality")
```
...
$I_0, I_1$ = constant multiplier and exponent for determining density-independent energy intake based on amount of structural biomass
...
b_t=0
# Increased natural mortality rate at R/S = 0 or S = 0 
...
lengths_at_t1 <- function(genotypes) {
  ...
  for (g in 1:genotypes) {
    ...
    for (i in 2:longevity) {
      new_R_l = get_pre_reproductive_size(age_dist[i-1+(longevity), 1],age_dist[i-1+2*(longevity), 1], i-1)
      age_dist[i+(longevity), 1] = new_R_l[1]
      age_dist[i+(2*longevity), 1] = new_R_l[2]
...
for (t in 2:runtime) {
  b_t=0
  if (t == round(add_genotypes/timescale)) {
    Fishing = log(exp(-0.2)^(1/(365/timescale)))*(-1)
  }
  ...
  for (genotype in 1:genotypes) {
    ...
    for (age in 2:longevity) {
      new_R_l = get_pre_reproductive_size(age_dist[age-1+(longevity), t-1],age_dist[age-1+2*(longevity), 1], age-1)
      age_dist[age+(longevity), t] = new_R_l[1]
      age_dist[age+(2*longevity), t] = new_R_l[2]
...
plot(approach[seq(4,length(approach), by=4)], col="black", pch=19, type="l", ylab="Abundance",xlab="Time (3 months)")
approach <- vector(mode="numeric", length=runtime)
for (i in 1:runtime) {
  approach[i] <- sum(age_dist_1[1:longevity,i])
}
lines(approach[seq(4,length(approach), by=4)], col="red", pch=19, type="l")
cols <- c("orange","green","blue","purple")
for (j in 2:genotypes) {
  approach <- vector(mode="numeric", length=runtime)
  for (i in 1:runtime) {
    approach[i] <- sum(get(paste("age_dist_",j, sep=""))[1:longevity,i])
  }
  lines(approach[seq(4,length(approach), by=4)], col=cols[j-1], pch=19)
}
```
