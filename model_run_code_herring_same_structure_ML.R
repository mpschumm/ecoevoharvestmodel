...
rand_resource_vec <- rep((sqrt(log(seq(0,8, length=14)^2 + 1))), each=7000,times=1) # Values input to seq() are in units of coefficient of variation of lognormal distribution of resource K - they are back-converted here to SD of a Normal
...
model_run <-function(list, x) {
  ...
  random_vec_summaries[[x]] <<- summary(exp(random_values - (sd(random_values)^2)/2))
